...
class LlamaCppTranslator:
    def __init__(self, api_url: str, timeout: int = 60):
        """
        初始化 LlamaCppTranslator 对象。
        
        :param api_url: llama.cpp 接口的地址。
    ...
    def generate_prompt(self, japanese, gpt_dict):
        """
        根据 gpt_dict 和输入文本生成翻译 prompt。
        
        :param gpt_dict: 包含术语表的列表。
        :param japanese: 要翻译的日文文本。
...
サービスのお客様からご利用いただきましたが、お客さまにお気になりますのですが、"""
    ...
    # 创建翻译器对象
    translator = LlamaCppTranslator(api_url)
